...
public partial class Index
{
    ...
    async Task GenerateAndDownloadFile()
    {
        ...
        if (settings.ConstantQueryParameterItems.Count > 0)
        {
            settings.ConstantQueryParameters = new Dictionary<string, string>();
            foreach (var item in settings.ConstantQueryParameterItems.Where(c => !string.IsNullOrEmpty(c.Key)))
            {
                settings.ConstantQueryParameters.Add(item.Key, item.Value);
            }
        }
