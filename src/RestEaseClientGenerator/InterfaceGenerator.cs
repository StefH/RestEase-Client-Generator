...
namespace RestEaseClientGenerator
{
    public class InterfaceGenerator
    {
        ...
        public ICollection<GeneratedFile> FromStream(Stream stream, string ns, string apiName, out OpenApiDiagnostic diagnostic)
        {
            ...
            var models = MapModels(openApiDocument.Components.Schemas, $"{ns}.Models");
            ...
            //File.WriteAllText($"../../../../{ns.Replace(".", "/")}/Models/{model.ClassName}.cs", modelText);

            var @interface = MapInterface(openApiDocument.Paths, apiName, ns);

            //File.WriteAllText($"../../../../{ns.Replace(".", "/")}/Api/{@interface.Name}.cs", interfaceResult);
        ...
        private static RestEaseInterfaceMethodDetails MapOperationToMappingModel(string path, string httpMethod, OpenApiOperation operation)
        {
            ...
            string methodRestEaseMethod = !string.IsNullOrEmpty(operation.OperationId) ? operation.OperationId.ToPascalCase() : methodRestEaseForAnnotation;
            ...
            var method = new RestEaseInterfaceMethodDetails
            {
                ...
                //$"Task{returnType} {methodRestEaseMethod}Async({string.Join(", ", methodParameterList)});"
            };
